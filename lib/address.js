...
addressFactory.generateNew = function(pubKeyHashVersion, checksumValue) {
    return _generateAddress(_generatePrivateKey(), pubKeyHashVersion, checksumValue, false);
};
...
Address.prototype.sign = function(rawTransaction, index, getInputScript) {
    ...
    console.log('unsigned to buffer', _toBuffer(rawTransaction).toString('hex'));

    var input = rawTransaction.vin[index];
    console.log(input);

    var ripemd160Hash =  crypto.ripemd160(crypto.sha256(this.publicKey));
    ...
    chunks.push(uint8Buffer(OPS.OP_CHECKSIG));
     ...
     var script = Buffer.concat(chunks);
    */

    return getInputScript(rawTransaction.vin[index]['txid'], rawTransaction.vin[index]['vout']).then((script) => {
        rawTransaction.vin[index].script = Buffer.from(script, 'hex');
        ...
        var signature = secp256k1.sign(hashForSignature, this.privateKey).signature;
        ...
        var scriptSig = Buffer.concat([pushDataIntBuffer(scriptSignature.length), scriptSignature, pushDataIntBuffer(this.publicKey.length), this.publicKey]);
...
_toBuffer = function(decodedTransaction) {
    ...
    decodedTransaction.vin.forEach(function (txIn, index) {
        ...
        if (txIn.script != null) {
            ...
            console.log('length', txIn.script.length);
            chunks.push(varIntBuffer(txIn.script.length));
            console.log('script', txIn.script.toString('hex'));

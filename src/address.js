...
_generateAddress = function(privKey, pubKeyHashVersion, checksumValue) {
    ...
    var checksum = _generateChecksum(extendedRipemd160),
        xorChecksum = xor(checksum, checksumValue);
...
_extendWithVersion = function(hash, versionHash, nbSpacerBytes) {
    ...
    if (index < hash.length) {
        extendedParts.push(hash.slice(index * nbSpacerBytes + index, hash.length))
    }
...
_extractVersion = function(extendedHash, versionLength, nbSpacerBytes) {
    ...
    for (; index < versionLength; index++) {
        ...
        hashParts.push(extendedHash.slice(index * nbSpacerBytes + index + 1, (index + 1) * nbSpacerBytes + index + 1));
    }
...
_generateChecksum = function(extendedHash) {
    return crypto.hash256(extendedHash).slice(0, 4);
};